...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-6 rounded-xl border-2 border-border/50 py-6 shadow-lg backdrop-blur-sm bg-card/80 hover:shadow-xl hover:border-border transition-all duration-300",
        className
      )}
      {...props}
    />
